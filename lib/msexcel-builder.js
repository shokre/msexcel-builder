 ...
 */
var ContentTypes, DocPropsApp, EasyZip, SharedStrings, Sheet, Style, Workbook, XlRels, XlWorkbook, async, exec, existsSync, fs, opt, path, tool, xml,
...
tool = {
  i2a: function(i) {
    var alphabet, getCellAdr, len;
    alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    len = alphabet.length;
    getCellAdr = function(num) {
      var output, pos, tmp;
      pos = num % len;
...
Sheet = (function() {
  ...
  Sheet.prototype.val = function(col, row, val) {
    return this.data[row][col].val = val;
  };
  ...
  Sheet.prototype.numFmt = function(col, row, formatCode) {
    return this.styles['numfmt_' + col + '_' + row] = this.book.st.numfmt2id(formatCode);
  };
...
Style = (function() {
  ...
  Style.prototype.fill2id = function(fill) {
    ...
    fill.bgRGB || (fill.bgRGB = '-');
    fill.fgRGB || (fill.fgRGB = '-');
  ...
  Style.prototype.numfmt2id = function(formatCode) {
    var fmt, id, k;
    fmt = {
      numFmtId: '',
    ...
    if (id) {
      ...
      this.numFmts.push(fmt);
      fmt.numFmtId = this.cache[k] = this.numFmts.length + 200;
      return fmt.numFmtId;
    }
  };

  Style.prototype.toxml = function() {
    var bders, cs, e, ea, es, fills, fonts, numfs, o, ss, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3, _ref4;
    ...
    numfs = ss.ele('numFmts', {
      count: this.numFmts.length
    });
    _ref = this.numFmts;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      o = _ref[_i];
...
Workbook = (function() {
  ...
  Workbook.prototype.save = function(cb) {
    var i, self, sheet, sheetList, target, _i, _len, _ref;
    ...
    self = this;
    return async.series([
      async.apply(fs.copy, opt.tmpl_path + '/tmpl', target), async.apply(fs.mkdir, target + '/xl/_rels'), async.apply(fs.mkdir, target + '/xl/worksheets'), async.apply(fs.writeFile, target + '/[Content_Types].xml', self.ct.toxml()), async.apply(fs.writeFile, target + '/docProps/app.xml', self.da.toxml()), async.apply(fs.writeFile, target + '/xl/workbook.xml', self.wb.toxml()), async.apply(fs.writeFile, target + '/xl/sharedStrings.xml', self.ss.toxml()), async.apply(fs.writeFile, target + '/xl/_rels/workbook.xml.rels', self.re.toxml()), async.apply(fs.writeFile, target + '/xl/styles.xml', self.st.toxml()), function(cb) {
        return async.forEach(sheetList, function(sheet, sheetDone) {
          return fs.writeFile(sheet.name, sheet.data, sheetDone);
        }, function(err) {
          return cb(err);
        });
      }
      ...
      zip = new EasyZip();
      return zip.zipFolder(target + '/.', function() {
        zip.writeToFile(path.join(self.fpath, self.fname));
        return fs.remove(target, function() {
          return cb(null, path.join(self.fpath, self.fname));
        });
      });
    });
  };
  ...
  Workbook.prototype.cancel = function() {
    return fs.remove(path.join(this.fpath, this.id));
  };
